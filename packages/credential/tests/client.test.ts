...
import { CantonVcClient } from '../src/client';
...
describe('CantonVcClient — constructor validation', () => {
  it('rejects an empty clientId', () => {
    expect(
      () => new CantonVcClient({ ...CLIENT_OPTIONS, clientId: '' }),
    ).toThrow(CantonVcOauthError);
  });

  it('rejects an empty redirectUri', () => {
    expect(
      () => new CantonVcClient({ ...CLIENT_OPTIONS, redirectUri: '' }),
    ).toThrow(CantonVcOauthError);
  });

  it('strips trailing slashes from the issuer', async () => {
    const storage = buildMemoryStorage();
    const client = new CantonVcClient({ ...CLIENT_OPTIONS, issuer: 'https://issuer.example.test/' }, storage);
    const built = await client.buildAuthorizeUrl({ scope: ['openid'] });
    expect(built.url.startsWith('https://issuer.example.test/api/v1/oauth/authorize')).toBe(true);
  });
});

describe('CantonVcClient — buildAuthorizeUrl', () => {
  it('includes response_type, client_id, scope, state, PKCE params', async () => {
    const storage = buildMemoryStorage();
    const client = new CantonVcClient(CLIENT_OPTIONS, storage);
  ...
  it('rejects an empty scope list', async () => {
    const client = new CantonVcClient(CLIENT_OPTIONS, buildMemoryStorage());
    await expect(client.buildAuthorizeUrl({ scope: [] })).rejects.toThrow(CantonVcOauthError);
  });
  ...
  it('persists state + verifier + redirect to storage under the clientId', async () => {
    ...
    const client = new CantonVcClient(CLIENT_OPTIONS, storage);
    const built = await client.buildAuthorizeUrl({ scope: ['openid'] });
    expect(await storage.getItem(`canton-vc.oauth.state.${CLIENT_OPTIONS.clientId}`)).toBe(built.state);
    expect(await storage.getItem(`canton-vc.oauth.verifier.${CLIENT_OPTIONS.clientId}`)).toBe(built.codeVerifier);
    expect(await storage.getItem(`canton-vc.oauth.redirect.${CLIENT_OPTIONS.clientId}`)).toBe(CLIENT_OPTIONS.redirectUri);
  });
});

describe('CantonVcClient — handleCallback', () => {
  it('validates state against storage and returns code + verifier', async () => {
    ...
    expect(await storage.getItem(`canton-vc.oauth.state.${CLIENT_OPTIONS.clientId}`)).toBeNull();
  });
...
describe('CantonVcClient — exchangeCode', () => {
  it('posts form-encoded body with code, verifier, client_id and client_secret', async () => {
    ...
    expect(urlArg).toBe('https://issuer.example.test/api/v1/oauth/token');
...
describe('CantonVcClient — getUserinfo', () => {
