/**
 * {@link CantonVcClient} — the main SDK entry point.
 *
...
import { CantonVcOauthError, type CantonVcOauthErrorCode } from './errors';
import {
...
const KNOWN_SERVER_ERRORS: ReadonlySet<CantonVcOauthErrorCode> = new Set([
  'invalid_request',
  'unauthorized_client',
  'access_denied',
...
export class CantonVcClient {
  ...
  constructor(options: CantonVcClientOptions, storage?: SdkStorage) {
    if (options.clientId.length === 0) {
      throw new CantonVcOauthError('invalid_request', 'clientId is required.');
    }
    if (options.redirectUri.length === 0) {
      throw new CantonVcOauthError('invalid_request', 'redirectUri is required.');
    }
  ...
  async buildAuthorizeUrl(options: AuthorizeOptions): Promise<AuthorizeUrl> {
    if (options.scope.length === 0) {
      throw new CantonVcOauthError('invalid_scope', 'At least one scope is required.');
    }
   ...
   */
  async getUserinfo(accessToken: string): Promise<CantonVcClaims> {
    const res = await this.fetchImpl(`${this.issuer}/api/v1/oauth/userinfo`, {
      method: 'GET',
      headers: { Authorization: `Bearer ${accessToken}` },
    }).catch((err) => {
      throw new CantonVcOauthError('network_error', 'userinfo request failed.', { cause: err });
    });
    if (!res.ok) {
      await this.throwFromResponse(res);
    }
    return (await res.json()) as CantonVcClaims;
  }
  ...
  private async postForm(path: string, body: URLSearchParams): Promise<Response> {
    const res = await this.fetchImpl(`${this.issuer}${path}`, {
      ...
      throw new CantonVcOauthError('network_error', `POST ${path} failed.`, { cause: err });
    });
    if (!res.ok) {
      await this.throwFromResponse(res);
    }
...
function mapServerError(code: string): CantonVcOauthErrorCode {
  if (KNOWN_SERVER_ERRORS.has(code as CantonVcOauthErrorCode)) {
    return code as CantonVcOauthErrorCode;
  }
