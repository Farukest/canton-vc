/**
 * Canton-VC OAuth SDK error taxonomy.
 *
 * Every helper in this package throws `CantonVcOauthError` (and only
...
export type CantonVcOauthErrorCode =
  ...
  | 'not_a_callback'
  | 'network_error'
  // verifyDisclosure-side failures (claims missing the on-chain bundle)
  | 'disclosure_blob_missing'
  | 'disclosure_contract_id_missing'
  | 'unknown_error';

export interface CantonVcOauthErrorOptions {
  /**
...
export class CantonVcOauthError extends Error {
  readonly code: CantonVcOauthErrorCode;
  readonly description?: string;
  readonly state?: string;

  constructor(
    code: CantonVcOauthErrorCode,
    message: string,
    options: CantonVcOauthErrorOptions = {},
  ) {
    super(message, options.cause !== undefined ? { cause: options.cause } : undefined);
    this.name = 'CantonVcOauthError';
    this.code = code;
...
export function isCantonVcOauthError(value: unknown): value is CantonVcOauthError {
  return value instanceof CantonVcOauthError;
}
