...
- End-to-end live mainnet smoke against Sumsub sandbox (`sbx:` API key, `id-and-liveness` level) — 16 phases including `createCredential` × 4, `Credential_PublicFetch`, `Credential_ArchiveAsHolder`, `RevokeCredential` with NFT cascade, `UpdateCredentials` (bulk refresh), `createKycNft`, and standalone `BurnNft`. All choices exercised on canton-vc-credential DAR v2.1.0 (`9eecc8d4... → 562bbc75...`) without skips.
